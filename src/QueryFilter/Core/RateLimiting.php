...
trait RateLimiting
{
    ...
    {
        if (!config('laroFilters.rate_limit.enabled')) {
            return;
        }


        /** @var RateLimiter $limiter */
        $limiter = App::make('laro.filters.limiter');
        
        $key = $this->resolveRateLimitKey();
        $maxAttempts = config('laroFilters.rate_limit.max_attempts', 60);
        $decayMinutes = config('laroFilters.rate_limit.decay_minutes', 1);
        ...
        if ($limiter->tooManyAttempts($key, $maxAttempts)) {
            ...
            throw new ThrottleRequestsException(
                config('laroFilters.rate_limit.error_message', 'Too many filter requests. Please try again later.'),
                null,
                $headers
            );
        }
        ...
        if (config('laroFilters.rate_limit.include_headers', true)) {
