...
namespace LaroFilters\QueryFilter\Queries;
...
abstract class BaseClause
{
    ...
    {
        if (config('laroFilters.log.has_keeping_query')) {

            $endTime = microtime(true);
            $executionTime = $endTime - $startTime;

            if (!empty(config('laroFilters.log.max_time_query'))) {

                if ($executionTime >= config('laroFilters.log.max_time_query')) {
    ...
    {
        Log::info('laroFilters query', [
            'query' => $query->toSql(),
            'binding' => $query->getBindings(),
            'time' => $executionTime,
            'type' => config('laroFilters.log.type'),
        ]);
    }
}
