...
namespace LaroFilters;

use LaroFilters\Command\MakeEloquentFilter;
use LaroFilters\Facade\LaroFilters;
...
use LaroFilters\QueryFilter\Factory\QueryFilterCoreFactory;
...
class ServiceProvider extends BaseServiceProvider
{
    ...
    public function register(): void
    {
        ...
        $this->app->singleton('laro.filters.limiter', function ($app) {
    ...
    {
        if ($this->app['config']->get('laroFilters.rate_limit.enabled', true)) {
    ...
    {
        $this->publishes([
            __DIR__ . '/config/config.php' => config_path('LaroFilters.php'),
        ]);
    }
     ...
     * @param \LaroFilters\QueryFilter\Factory\QueryFilterCoreFactory $queryFilterCoreFactory
     * @return void
     */
    private function attachFilterToQueryBuilder(\Closure $mainQueryFilterBuilder, QueryFilterCoreFactory $queryFilterCoreFactory): void
    {
        \Illuminate\Database\Query\Builder::macro('filter', function ($request = null) use ($mainQueryFilterBuilder, $queryFilterCoreFactory) {
            ...
            return app('LaroFilters')->apply(builder: $this, request: $request);
        });
    }
    ...
    private function attachResponseFilterToQueryBuilder(\Closure $mainQueryFilterBuilder, QueryFilterCoreFactory $queryFilterCoreFactory): void
    {
        \Illuminate\Database\Query\Builder::macro('getResponseFilter', function ($callback = null) use ($mainQueryFilterBuilder, $queryFilterCoreFactory) {
            ...
            if (!empty($callback)) {
                return call_user_func($callback, LaroFilters::getResponse());
            }
