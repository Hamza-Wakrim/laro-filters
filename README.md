Laro Filters is a robust Laravel package providing an intuitive way to filter your Eloquent models using query
strings.
...
We've tailored Laro Filters to be as flexible as you need—whether your queries are straightforward or complex.
...
![](https://raw.githubusercontent.com/hamza-wakrim/laro-filters/master/Aban-21-1402%2014-27-04.gif)
...
Laro Filters can help you to manage these features. Just you will set query string to work with that.
...
2- Add `laroFilters\ServiceProvider::class` to provider `app.php`
   ...
   'providers' => [
      ...
      */
      laroFilters\ServiceProvider::class
   ]
   ```
...
'alias' => [
   ...
   * Facade alias...
   */
    'laroFilters' => laroFilters\Facade\LaroFilters::class,
],
```
...
| SpecialCondition         |                 | f_params[limit]=10<br/>f_params[orderBy][field]=id<br/>f_params[orderBy][field]=profile.name | support f_params, e.g:<br/> limit and orderBy<br/>(including relation sorting) | ✅         | ✅    |
...
| WhereLikeCondition       | where           | first_name[like]=Hamza                                       | where('first_name',<br/> 'like', $value)                                    | ✅         | ✅    |
| WhereInCondition         | whereIn         | username[]=Hamza&<br/>username[]=Hamza12                     | whereIn('username', $value)                                               | ✅         | ✅    |
| WhereOrCondition         | orWhere         | username=Hamza&<br/>or[username]=Hamza                       | orWhere('username', $value)                                               | ✅         | ✅    |
...
| WhereCondition           | where           | username=Hamza                                               | where('username', $value)                                                 | ✅         | ✅    |
...
- Only single-level relations are supported (e.g., `profile.name`, not `user.profile.name`)
- Relation sorting only works with Eloquent models, not DB queries
- If relation doesn't exist, throws `EloquentFilterException` with code `3`
...
- `LaroFilters::filterRequests()` : get all params that used by the Laro Filters.
  You can set key to get specific index.
  For example `LaroFilters::filterRequests('username')` it's getting username index.
...
- `LaroFilters::getRequestEncoded()` : get passed request as encoded request based on default.
- `LaroFilters::setRequestEncoded()` : set request encoded with a selective salt.
...
Sometimes you want to make your custom condition to make a new query that Laro Filters doesn't support by default.
...
- Laro Filters recognizes every param of the queries string.
  Maybe you have a query string that you don't want to recognize by Laro Filters. You can use `ignoreRequest` for his
  purpose.
...
If you'd like to contribute to Laro Filters, please fork the repository and create a pull request. We welcome
...
Laro Filters is open-source software licensed under the [MIT license](https://opensource.org/licenses/MIT).

## Contact

If you have any questions or feedback about Laro Filters, please feel free to contact us at
hamzawakrimm.developer@gmail.com. We'd love to hear from you!
